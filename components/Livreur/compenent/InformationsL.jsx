...
export default function InformationsL() {
  ...
  const fetchProfileImage = async () => {
    try {
      ...
      const responses2 = await axios.get(`http://192.168.1.109:8081/user/afficherPhotoProfile/${id}`, {
        headers: {
    ...
    try {
      ...
      if (status === 'granted') {
        ...
        if (!response.cancelled && response.assets[0].uri) {
          ...
          const responses = await axios.post(`http://192.168.1.109:8081/user/ajouterPhotoProfile/${id}`, formData, {
            headers: {
