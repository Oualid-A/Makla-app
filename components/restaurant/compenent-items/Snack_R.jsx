  ...
  useEffect(() => {
    const fetchCommands = async () => {
      try {
        ...
        const response2 = await axios.get(`http://192.168.1.109:8081/admin/getIdRestaurant/${id}`, {
          headers: {
        ...
        const response = await axios.get(`http://192.168.1.109:8081/commande/AllCommandeByRestaurantId/${id2}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    Alert.alert(`Demmande de ${id}`, " est diffuser", [
      {
        ...
        onPress: async () => {
          try {
            const token = await AsyncStorage.getItem('token');
            // const token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJ6YWtpaWlAZ21haWwuY29tIiwiZXhwIjoxNzAxNzkyMzczfQ.aEyfx7ZV8dlGlLrAziI6DQoOKoq1Yf3TDMAsZ4cbrF_OUX1IfsUYs_OcP5FQjL0vHoFwABCjMLb1Ghy1SuhTVA';
            const response = await axios.put(`http://192.168.1.109:8081/commande/modifierCommande/${id}`, {}, {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
